...
public class PrivacyAroundAdvice {
    ...
    public static Object execute(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (!ArrayUtils.isEmpty(pseArray)) {
                ...
                // after
                if(Mono.class.isAssignableFrom(o.getClass())) {
                    if(isGetMethod(methodName)) {
                        return ReactiveAuthenticationHolder
                                .get()
                                .flatMap(auth -> {
                                    Mono<?> result = (Mono<?>) o;
                                    // Due to the problem of authority, it should be handled separately
                                    return result.map( obj-> {
                                        handleGetInfo(methodName, obj, auth.getUser().getUsername(), auth.getUser().getId());
                                    return obj;
                                    });
                                });
                    } else {
                        Mono<?> result = (Mono<?>) o;
                        return result.map( obj-> executeAopAfterWithoutGet(methodName, obj, cycleStepExecuted));
                    }
                } else if(Flux.class.isAssignableFrom(o.getClass())) {
                    Flux<?> result = (Flux<?>) o;
                    return result.map( obj-> executeAopAfterFull(methodName, obj, cycleStepExecuted));
                } else {
    ...
    private static Object executeAopAfterWithoutGet(String methodName, Object o, boolean[] cycleStepExecuted) {
    ...
    private static Object executeAopAfterFull(String methodName, Object o, boolean[] cycleStepExecuted) {
        ...
        return executeAopAfterWithoutGet(methodName, o, cycleStepExecuted);
    }
    ...
    private static void parametersHandle(String infoMethodName, Object[] objects,
        ...
        if(isNumeric(firstStr)) {
            ...
            if(infoMethodName.contains(line)) {
                ...
                Object res = monoFluxHandle(objects[realInfoPosition], realInfoMethodName,
                        objects[idPosition], idMethodName, sensitivePee, isGrant, isVerify, true, cycleEnum);
                if(res == null) {
                    // res -> not mono/flux, so next -> handle
                    objects[idPosition] = monoFluxHandle(objects[idPosition], idMethodName,
                            objects[realInfoPosition], realInfoMethodName, sensitivePee, isGrant, isVerify, false, cycleEnum);
                }
                return;
            }
            ...
            objects[position] = monoFluxHandle(objects[position], infoMethodName,
                    null, null, sensitivePee, isGrant, isVerify, false, cycleEnum);
        } else {
            // after need execute
    ...
    private static Object monoFluxHandle(Object objPosition, String infoMethodName,
                                         Object objSpare, String spareMethodName,
                                         PrivacyElemEntity sensitivePee, Boolean isGrant, Boolean isVerify,
                                         Boolean hasNext, PrivacyCycleEnum cycleEnum) {
        // Both objects must not be mono/flux type; maybe one is not, or neither
        // mono/flux handle
        if(Mono.class.isAssignableFrom(objPosition.getClass())) {
            Mono<?> mono = (Mono<?>) objPosition;
            return mono.map(o -> {
                if(isVerify) {
                    return setVerifyOfObject(objPosition, infoMethodName, sensitivePee.getFieldInfo());
                }
                // objSpare must not mono type
                if(objPosition.equals(objSpare)) {
                    callOperateByEnum(o, infoMethodName, o, spareMethodName, sensitivePee, isGrant, cycleEnum);
                }
                callOperateByEnum(o, infoMethodName, objSpare, spareMethodName, sensitivePee, isGrant, cycleEnum);
                return o;
            });
        } else if(Flux.class.isAssignableFrom(objPosition.getClass())) {
            Flux<?> flux = (Flux<?>) objPosition;
            return flux.map(o -> {
                if(isVerify) {
                    return setVerifyOfObject(objPosition, infoMethodName, sensitivePee.getFieldInfo());
                }
                // objSpare must not flux type
                if(objPosition.equals(objSpare)) {
                    callOperateByEnum(o, infoMethodName, o, spareMethodName, sensitivePee, isGrant, cycleEnum);
                } else {
                    callOperateByEnum(o, infoMethodName, objSpare, spareMethodName, sensitivePee, isGrant, cycleEnum);
            ...
            if (hasNext) {
                // next processing, skip this time
                return null;
            }
            callOperateByEnum(objPosition, infoMethodName, objSpare, spareMethodName, sensitivePee, isGrant, cycleEnum);
            return objPosition;
        }
    }
